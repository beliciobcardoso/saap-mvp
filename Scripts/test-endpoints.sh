...
# No máximo 5 combinações: LoginRateLimitFilter bloqueia por IP após 5
# tentativas/minuto, e ir além disso apenas autobloqueia o próprio script.
CREDENTIALS=(
  "admin@saap.com:adminPass123"
  "admin@saap.com:password123"
  "receptionist@saap.com:password123"
  "receptionist@saap.com:test"
  "user@test.com:test"
)
...
for CRED in "${CREDENTIALS[@]}"; do
  ...
  http_response=$(curl -s -w '\n%{http_code}' -X POST "$BASE_URL/auth/login" \
    ...
    -d "{\"email\":\"$EMAIL\",\"password\":\"$PASSWORD\"}" 2>/dev/null || echo -e "{}\n000")
  http_code=$(echo "$http_response" | tail -1)
  response=$(echo "$http_response" | sed '$d')

  if [ "$http_code" = "429" ]; then
    log_error "Rate limit de login atingido (429) antes de achar credenciais válidas"
    log_info "Aguarde 1 minuto e rode o script novamente"
    exit 1
  fi
...
confirm_http_response=$(curl -s -w '\n%{http_code}' -X PUT "$BASE_URL/appointments/$APPOINTMENT_ID/confirm" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  -H "Content-Type: application/json")
confirm_http_code=$(echo "$confirm_http_response" | tail -1)
confirm_response=$(echo "$confirm_http_response" | sed '$d')

if [ "$confirm_http_code" != "200" ]; then
  log_error "Falha ao confirmar agendamento (HTTP $confirm_http_code)"
