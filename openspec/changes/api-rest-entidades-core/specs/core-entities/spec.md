...
- **WHEN** o cliente HTTP envia um payload JSON válido via POST para `/api/patients`
...
- **WHEN** o cliente HTTP envia uma requisição DELETE para `/api/professionals/{id}`
...
- **WHEN** o cliente envia uma requisição POST para `/api/patients` com o campo CPF contendo menos ou mais de 11 dígitos
...
- **WHEN** o cliente envia uma requisição POST para `/api/patients` com o campo email no formato "email_invalido.com"
- **THEN** o sistema SHALL rejeitar a requisição e retornar status HTTP 400 (Bad Request) detalhando a falha de validação do campo
...
- **WHEN** o cliente envia um POST para `/api/users` com um e-mail que já existe no banco de dados
